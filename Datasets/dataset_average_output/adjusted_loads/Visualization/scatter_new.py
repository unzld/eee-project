...
df = pd.read_csv('d3_avg_error_adjustedloads.csv')
...
f, (ax) = plt.subplots(1, figsize=(8, 4), dpi=150)
sns.scatterplot(data=df.reset_index(), ax=ax, x="index", y="thief_val", hue='label', s=10)
#sns.scatterplot(data=df, ax=ax, x="min", y="thief_val", hue='label', s=10)
ax.axhline(y = 0, color = 'dimgray', linestyle = 'dotted')
ax.set_xlabel("Minimum % Loss Error",size = 12,alpha=0.8)
ax.set_ylabel("% Loss Error of CM with Pilferer",size = 12,alpha=0.8)
ax.set_title('Dataset 3: % Loss Error of CM with Pilferer')
